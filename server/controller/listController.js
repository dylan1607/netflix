...
//Delete All list movie
const deleteAllList = async (req, res) => {
  if (req.user.isAdmin) {
    try {
      await List.deleteMany({});
      res.status(201).json("All List Movie has been deleted ...");
      console.log("All List movie has been deleted !");
    } catch (error) {
      res.status(500).json(error);
      console.error("Delete All List failed");
    }
  } else {
    res.status(403).json("You are not allowed !");
  }
